                ...
                <AnimatePresence>
                  {isDialogOpen && (
                    <DialogContent className="sm:max-w-[425px] bg-white/80 backdrop-blur-md dark:bg-gray-800/80">
                      ...
                      >
                        <DialogHeader>
                          <DialogTitle className="text-2xl font-bold text-primary">Pronto para a aventura?</DialogTitle>
                          <DialogDescription className="text-lg">
                            Você está prestes a iniciar um quiz incrível sobre <span className="font-bold text-accent">{selectedCategory?.name}</span>!
                          </DialogDescription>
                        </DialogHeader>
                        <div className="mt-6 flex flex-col items-center">
                          <img src="geo-mascote-transparente.svg" alt="Quiz Mascot" className="w-32 h-32 mb-4 animate-bounce" />
                          <p className="text-center mb-4">Nosso mascote está animado para ver como você se sai!</p>
                          <div className="flex justify-center space-x-4 w-full">
                            <Button variant="outline" onClick={() => setIsDialogOpen(false)} className="w-1/3">
                              Ainda não
                            </Button>
                            <Button onClick={handleStartQuiz} className="w-2/3 bg-gradient-to-r from-green-400 to-blue-500 hover:from-green-500 hover:to-blue-600 text-white font-bold py-2 px-4 rounded-full transition-all duration-300 transform hover:scale-105">
                              Vamos nessa!
                            </Button>
                          </div>
                        </div>
